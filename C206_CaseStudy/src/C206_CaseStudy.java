...
public class C206_CaseStudy {
		...
		while (option != 5) {
			...
			if (option == 1) { // administrator
				...
				if (adOption == 1) { // schoolManagement
					...
					if (schOption == 1) {
						// Add a school
						SchoolMethods.addSchool(SchoolList, SchoolMethods.inputSchool());
						System.out.println("School added");

					} else if (schOption == 2) {
						// View all schools
						SchoolMethods.viewAllSchool(SchoolList);

					} else if (schOption == 3) {
						// Delete a school
						String nameOfSchooltoDelete = Helper.readString("Enter School Name to delete: ");
						SchoolMethods.deleteSchoolByName(SchoolList, nameOfSchooltoDelete);
	...
	public static String retrieveAllVendor(ArrayList<Vendor> VendorList) {
		...
		for (int i = 0; i < VendorList.size(); i++) {
			...
			output += String.format("%-20s %-25s %-27s %-10s\n", VendorList.get(i).getVendorName(),
					VendorList.get(i).getDescription(), VendorList.get(i).getContactNumber(),
	...
	public static void viewAllVendor(ArrayList<Vendor> VendorList) {
		...
		String output = String.format("%-20s %-25s %-27s %-10s\n", "VENDOR NAME", "VENDOR DESCRIPTION",
				"VENDOR CONTACT NUMBER", "VENDOR ADDRESS");
